...
export function PhaseSummary({ campaign, role, logs, isHistoryView = false }: PhaseSummaryProps) {
  ...
  const handleCopyMarkdown = () => {
    const summary = generateMarkdownSummary(role, campaign, phaseLogs);
    navigator.clipboard.writeText(summary);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      {/* ─── Hero Header ─── */}
      <div className="text-center space-y-3 py-6 print:py-0">
        <div className="inline-block px-3 py-1 rounded-full bg-legion-amber/10 border border-legion-amber/30 text-[10px] text-legion-amber font-bold uppercase tracking-[0.2em] mb-2 print:text-black print:border-black">
          Phase {currentPhase} Complete
        </div>
        <h2 className="font-heading text-4xl text-legion-text-primary uppercase tracking-tight print:text-black">
          The Long Retreat
        </h2>
        <p className="text-sm text-legion-text-muted italic max-w-md mx-auto leading-relaxed print:text-black">
      ...
      <LegionCard className="border-legion-amber/40 shadow-xl shadow-legion-amber/5 print:shadow-none print:border-black print:text-black">
        <LegionCardHeader className="border-b border-white/5 pb-4 bg-white/5 print:bg-transparent print:border-black">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded bg-legion-amber/20 print:hidden">
                <HistoryIcon className="w-5 h-5 text-legion-amber" />
              </div>
              <div>
                <LegionCardTitle className="text-lg font-heading text-legion-text-primary uppercase tracking-widest print:text-black">
                  {role} Report
                </LegionCardTitle>
                <p className="text-[10px] text-legion-text-muted uppercase tracking-[0.1em] font-mono print:text-black">
        ...
        <LegionCardContent className="pt-8 space-y-10 print:space-y-6">
          
          {/* Global Impact (Deltas) */}
          <section className="space-y-4">
            <h4 className="text-[11px] font-mono uppercase tracking-[0.2em] text-legion-amber border-b border-legion-amber/10 pb-1 print:text-black print:border-black">Strategic Impact</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 print:gap-6 print:grid-cols-1">
            <RoleNarrative role={role} logs={phaseLogs} />
            
            {/* Highlights */}
            <section className="space-y-4">
              <h4 className="text-[11px] font-mono uppercase tracking-[0.2em] text-legion-text-muted border-b border-white/5 pb-1 print:text-black print:border-black">Phase Highlights</h4>
              <div className="space-y-3">
                {getHighlights(role, phaseLogs).map((h, i) => (
                  <div key={i} className="flex gap-3 text-xs leading-relaxed group">
                    <span className="text-legion-amber mt-1 shrink-0 print:text-black"><ZapIcon className="w-3 h-3" /></span>
                    <p className="text-legion-text-muted group-hover:text-legion-text-primary transition-colors print:text-black">{h}</p>
                  </div>
                ))}
              </div>
            </section>
...
function getRoleNarrative(role: LegionRole, logs: CampaignPhaseLog[]): string {
  switch (role) {
    ...
    case 'SOLDIER': return "You are the blood and bone of the Legion. While you do not hold a command role, your survival and your witness are what keep the Legion alive. You follow the march, watching as the horizon shifts and the Chosen lead the way.";
...
function generateMarkdownSummary(role: LegionRole, campaign: Campaign, logs: CampaignPhaseLog[]): string {
